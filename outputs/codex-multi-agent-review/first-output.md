...
Public Sub ImportMonthlyClaimCounts()
    ...
    If Not TryParseMonthlyFileName(wb.Name, targetYYMM, targetYear, targetMonth) Then
        AddLog "月次ファイル名形式違い", wb.FullName, "", 0, "", "", "", _
    ...
    BuildMasterSets wsMaster, masterBranches, masterBusinesses
    BuildMonthlyIndex wsMonthly, detailRows, groupDetails, subtotalRows
    ResetMonthlyArea wsMonthly, daysInMonth

    If Not fso.FolderExists(dailyRoot) Then
        AddLog "日次フォルダなし", dailyRoot, "", 0, "", "", "", _
...
FatalError:
    ...
    AddLog "致命的エラー", ThisWorkbook.FullName, "", 0, "", "", "", _
...
Private Sub PrepareLogSheet(ByVal ws As Worksheet)
    ...
    ws.Range("A1:I1").Value = Array("発生日時", "種別", "ファイルパス", "シート名", "行番号", _
                                    "支店コード", "業務コード", "日付", "内容")
    mLogRow = 2
    mLogCount = 0
End Sub
...
Private Sub AddLog(ByVal logType As String, ByVal filePath As String, ByVal sheetName As String, _
                   ...
                   ByVal targetDate As String, ByVal message As String)
...
Private Sub BuildMasterSets(ByVal ws As Worksheet, ByVal branches As Object, ByVal businesses As Object)
    ...
    branchCol = FindHeaderColumn(ws, "支店コード")
    businessCol = FindHeaderColumn(ws, "業務コード")

    If branchCol = 0 Or businessCol = 0 Then
        AddLog "マスタ見出し不足", ThisWorkbook.FullName, ws.Name, 1, "", "", "", _
               "マスタシートに 支店コード または 業務コード の見出しが見つかりません。"
        Err.Raise vbObjectError + 1002, , "マスタシートの見出しが不足しています。"
    End If
...
Private Function FindHeaderColumn(ByVal ws As Worksheet, ByVal headerText As String) As Long
    ...
    For c = 1 To lastCol
        If Trim$(CStr(ws.Cells(ROW_HEADER, c).Value)) = headerText Then
            FindHeaderColumn = c
            Exit Function
        End If
    Next c
        ...
        Select Case rowType
            Case "DETAIL"
                ...
                If Len(branchCode) = 0 Or Len(businessCode) = 0 Then
                    AddLog "月次DETAILキー不備", ThisWorkbook.FullName, ws.Name, r, branchCode, businessCode, "", _
                           "DETAIL行の支店コードまたは業務コードが空欄です。"
                Else
                    key = MakeKey(branchCode, businessCode)
                    If detailRows.Exists(key) Then
                        AddLog "月次DETAIL重複", ThisWorkbook.FullName, ws.Name, r, branchCode, businessCode, "", _
            ...
            Case "SUBTOTAL"
                If Len(groupName) = 0 Then
                    AddLog "月次SUBTOTALグループ不備", ThisWorkbook.FullName, ws.Name, r, "", "", "", _
...
Private Sub ResetMonthlyArea(ByVal ws As Worksheet, ByVal daysInMonth As Long)
    Dim lastRow As Long
    Dim r As Long
    Dim rowType As String
...
Private Sub ProcessDailyFile(ByVal filePath As String, ByVal fileName As String, ByVal targetYYMM As String, _
    ...
    If Not TryParseDailyFileName(fileName, yymmdd, fileYYMM, fileYear, fileMonth, fileDay) Then
        AddLog "日次ファイル名形式違い", filePath, "", 0, "", "", "", _
               "日次ブック名は クレーム集計YYMMDD.xlsx 形式にしてください。"
        Exit Sub
    End If

    If fileYYMM <> targetYYMM Then
        AddLog "対象月違い", filePath, "", 0, "", "", yymmdd, _
    ...
    If processedDates.Exists(yymmdd) Then
        AddLog "同じ日付の重複日次ファイル", filePath, "", 0, "", "", targetDateText, _
    ...
    If Not WorksheetExists(dailyWb, SHEET_DAILY) Then
        AddLog "日次集計シートなし", filePath, "", 0, "", "", targetDateText, _
               "日次ブックに 日次集計 シートがありません。"
        GoTo FileExit
    End If

    adopted = True
    processedDates.Add yymmdd, filePath
...
FileError:
    ...
    AddLog "日次ブック処理エラー", filePath, "", 0, "", "", targetDateText, _
...
Private Sub ProcessDailyRows(ByVal ws As Worksheet, ByVal filePath As String, ByVal targetDateText As String, _
    ...
    For r = ROW_DATA_START To lastRow
        ...
        If statusText <> "ok" Then GoTo ContinueRow

        countValue = ws.Cells(r, DAILY_COL_COUNT).Value

        If IsError(countValue) Then
            AddLog "クレーム件数非数値", filePath, ws.Name, r, branchCode, businessCode, targetDateText, _
                   "クレーム件数がエラー値です。"
            GoTo ContinueRow
        End If

        countText = Trim$(CStr(countValue))

        If Len(countText) = 0 Then
            AddLog "クレーム件数空欄", filePath, ws.Name, r, branchCode, businessCode, targetDateText, _
                   "クレーム件数が空欄です。"
            GoTo ContinueRow
        End If
        ...
        If Not branchOk Then
            AddLog "未知の支店コード", filePath, ws.Name, r, branchCode, businessCode, targetDateText, _
                   "マスタに存在しない支店コードです。"
        End If

        If Not businessOk Then
            AddLog "未知の業務コード", filePath, ws.Name, r, branchCode, businessCode, targetDateText, _
                   "マスタに存在しない業務コードです。"
        End If
        ...
        If Not detailRows.Exists(key) Then
            AddLog "月次表に存在しないキー", filePath, ws.Name, r, branchCode, businessCode, targetDateText, _
...
Private Sub RecalculateMonthlyTotals(ByVal ws As Worksheet, ByVal groupDetails As Object, _
    ...
    For r = ROW_DATA_START To lastRow
        ...
        If rowType = "DETAIL" Then
            ws.Cells(r, COL_MONTH_TOTAL).Value = Application.WorksheetFunction.Sum( _
    ...
    For Each subtotalGroup In subtotalRows.Keys
        ...
        For Each subtotalRow In subtotalList
            ...
            For dayIndex = 1 To daysInMonth
                ...
                ws.Cells(CLng(subtotalRow), COL_FIRST_DAY + dayIndex - 1).Value = dayTotal
                total = total + dayTotal
            Next dayIndex

            ws.Cells(CLng(subtotalRow), COL_MONTH_TOTAL).Value = total
