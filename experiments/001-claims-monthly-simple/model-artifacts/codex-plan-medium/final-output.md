# codex-plan-medium first output

## 短い計画

1. 月次ブックの `月次集計` シートで、支店コード + 業務コードから転記先行を引けるようにする。
2. 転記前に `E:AI` の日別列と `AJ` の月合計をクリアして、再実行時の二重加算を防ぐ。
3. `ThisWorkbook.Path\daily\2605\` の日次ファイルを順に開き、ファイル名 `YYMMDD` から日付列を決める。
4. 各日次ファイル内で支店コード + 業務コードごとに件数を合算してから、月次表へ転記する。
5. 転記後に月合計を再計算し、処理結果とエラーは `処理ログ` シートへ記録する。

## 実装方針

- 入口プロシージャは `RunClaimsMonthlyTransfer`。
- VBAは標準モジュールに貼り付ける。
- 月次ブックと同じフォルダに `daily\2605\` がある前提で、実ローカルパスは固定しない。
- 追加参照は不要。キー管理には `CreateObject("Scripting.Dictionary")` のlate bindingを使う。
- MsgBoxは使わず、ログシートへ `日時 / レベル / 対象 / 内容` を出力する。
- 未登録キー、読めない日次ファイル、日付を判断できないファイルはログへ残して処理を継続する。
- 実行開始時に対象範囲をクリアするため、同じブックで再実行しても二重加算しない。
...
`RunClaimsMonthlyTransfer`

## 配置場所

月次ブック `月次クレーム集計2605.xlsm` の標準モジュールに貼り付けます。サンプル `.xlsx` は入力確認用であり、モデル成果物として `.xlsm` は保存しません。

## 実行方法

1. 月次ブックをマクロ有効ブックとして開く。
2. 月次ブックと同じフォルダ配下に `daily\2605\` があり、日次ファイル `クレーム集計260501.xlsx` などが置かれていることを確認する。
3. VBAエディタで標準モジュールにコードを貼り付ける。
4. `RunClaimsMonthlyTransfer` を実行する。

## 参照設定要否

追加参照は不要です。`Scripting.Dictionary` はlate bindingで生成するため、Microsoft Scripting Runtimeの参照設定は不要です。

## ログ出力

`処理ログ` シートを作成または初期化し、処理開始、クリア、日次ファイルごとの処理結果、未登録キー、エラー、完了を一覧形式で記録します。MsgBoxでは停止しません。

## エラー処理

日次ファイル単位のエラーはログへ記録して該当ファイルを閉じ、次の処理へ進みます。致命的エラーも可能な範囲で `処理ログ` に記録し、Excelの画面更新とイベント設定を戻して終了します。

## 再実行安全性

実行開始時に月次表の `E2:AJ最終行` をクリアしてから、日次ファイルを読み直して値を書き込みます。そのため同じブックで2回実行しても、既存値に加算され続けることはありません。
